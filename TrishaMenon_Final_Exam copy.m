...
d2_0 = d_from_sight(a0,b0);    % Using the d_from_sight function I created,
d2_3 = d_from_sight(a3,b3);      % these distances are calculated based on 
d2_4 = d_from_sight(a4,b4);      % the angles between the sighting and the 2 stations
...
y0 = y_coor(b0, d2_0);           % coordinates of the projectile at each
x3 = 3;                          % sighting using the function y_coor which
y3 = y_coor(b3, d2_3);           % finds the y-coordinate based on the angle
x4 = 4;                          % from Station 2 and the distance between
y4 = y_coor(b4, d2_4);           % projectile at each sighting and the Station

Vx = d2_3/(x3 - x0);           % Vx represents the horizontal velocity of the 
                                 % projectile based on the formula given

get_time_func(0, y0, 3, y3, 4, y4); % The get_time_func produces the function
